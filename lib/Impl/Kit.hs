...
represent the untyped lambda calculus. It is similar to the DeBruijnPar
...
data Semantics (e :: Con -> *) (m :: Con -> *) =
  Semantics { weak  :: forall g d. Included g d -> e g -> e d 
...
wkEnv :: forall h d g e m. Semantics e m -> Included d h 
      -> Environment d e g -> Environment h e g
wkEnv Semantics{..} inc env v = weak inc $ env v

semanticsTerm :: forall e m d g. Semantics e m -> Term g
              -> Environment d e g -> m d 
...
evalTerm sem@Semantics{..} g t = semanticsTerm sem t (env g) where
...
prettyPrinting =
  Semantics { weak  = \ _ -> Constant . runConstant
...
fromLC = to [] 
  where to :: [(IdInt, Var n)] -> LC IdInt ->  Term n
        to vs (Var v)   = TeVar (fromJust (lookup v vs))
        to vs (Lam v b) = TeLam b' where
