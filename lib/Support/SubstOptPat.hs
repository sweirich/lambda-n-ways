...
-- includes Pattern binding and binding multiple sorts
-- (such as type and term variables)
module Support.SubstOptPat
  ( VarC (..),
    AlphaC (..),
    SubstC (..),
    Var (..),
...
-- A *single* substitution for a free variable
data SubFv = forall a. Typeable a => SubFv a

newtype SubFvs = SubFvs (M.IdIntMap Dynamic)
...
-- | Type for operations on syntax
-- These operations are recursive over the structure of terms and
-- may be indexed by a binding level "k"
class Typeable a => AlphaC a where
...
class AlphaC a => SubstC b a where
  ...
  multi_subst_bv k vs x =
    case isvar x of
      Just v -> multiSubstBvVar k vs v x
...
multiSubstBvVar :: VarC a => Int -> [a] -> Var -> a -> a
multiSubstBvVar k vs v@(B i) _ = nthWithDefault (var v) vs (i - k)
multiSubstBvVar _ _ _ a = a
{-# INLINEABLE multiSubstBvVar #-}

-- | multi substitution for a single free variable
multiSubstFvVar :: VarC a => M.IdIntMap a -> Var -> a -> a
multiSubstFvVar m v@(F x) _ = M.findWithDefault (var v) x m
multiSubstFvVar _ _ a = a

-- | single substitution for a single bound variable (0)
substBvVar :: VarC a => a -> Var -> a -> a
substBvVar u = multiSubstBvVar 0 [u]

-- | single substitution for a single free variable
substFvVar :: VarC a => a -> IdInt -> Var -> a -> a
substFvVar u y (F x) _ | x == y = u
substFvVar _ _ _ a = a
