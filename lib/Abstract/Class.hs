...
data LC v = Var !v | Lam !(Bnd v) | App !(LC v) !(LC v)
  deriving (Generic)
...
class
  ...
  where
  type Bnd v :: Type
  type BindM v :: Type -> Type

  -- ambiguous. must use @v
  runBindM :: BindM v a -> a

  aeq :: LC v -> LC v -> BindM v Bool

  bind :: v -> LC v -> BindM v (Bnd v)
  unbind :: Bnd v -> BindM v (v, LC v)

  toLC :: LC v -> BindM v (LC IdInt)
  fromLC :: LC IdInt -> BindM v (LC v)

  type Subst v :: Type -> Type
  singleton :: v -> LC v -> Subst v (LC v)
  subst :: Subst v (LC v) -> LC v -> BindM v (LC v)

  -- some implementations may choose to override this definition with something
  -- faster
  instantiate :: Bnd v -> LC v -> BindM v (LC v)
  instantiate b a = do
    (x, e) <- unbind b
    subst (singleton x a) e

instance (NFData a, NFData (Bnd a)) => NFData (LC a)

instance BindingImpl v => Eq (LC v) where
  (==) = \x y -> runBindM @v (aeq x y)

lam :: BindingImpl v => v -> LC v -> BindM v (LC v)
...
nf e = runBindM @v (nf' @v e)

nf' :: forall v. BindingImpl v => LC v -> BindM v (LC v)
nf' e@(Var _) = return e
nf' (Lam b) = do
  (x, e) <- unbind @v b
  e' <- nf' e
...
whnf :: BindingImpl v => LC v -> BindM v (LC v)
...
instrInstantiate :: forall v. (BindingImpl v) => Bnd v -> LC v -> M v (LC v)
...
bind' :: forall v. (BindingImpl v) => v -> LC v -> Bnd v
bind' v a = runBindM @v (bind v a)

unbind' :: forall v. (BindingImpl v) => Bnd v -> (v, LC v)
