...
instance Show (Fin n) where
  show First = "FZ"
  show (Next n) = "(FS " ++ show n ++ ")"

instance NFData (Fin a) where
  rnf First = ()
  rnf (Next s) = rnf s
...
liftVar :: Fin n -> Fin (Pred n) -> Fin n
liftVar x y =
  case x of
    First -> Next y
    Next x' -> case y of
      First -> First
      Next y' -> Next (liftVar x' y')
...
substVar :: Fin n -> Fin n -> Maybe (Fin (Pred n))
substVar x = case x of
  ...
  Next x' -> \y ->
    ( case y of
        First -> case (nzf x') of Refl -> Just First -- [nzf x' return n, fin n for First]
        Next y' ->
          case substVar x' y' of
            Nothing -> Nothing
            Just f -> case (nzf y') of Refl -> Just (Next f) -- [nzf y' return n, fin n for Next f]
    )
...
subst :: DB n -> Fin n -> DB (Pred n) -> DB (Pred n)
subst e f v = case e of
  ...
  DLam e1 ->
    case (nzf f) of
      Refl -> DLam (subst e1 (Next f) (lift v First))

instantiate :: DB (S n) -> DB n -> DB n
instantiate b a = subst b First a
